...
// ops.PipelineRun ustvari OperationsRun.BeginAsync, ciscenje pa ga do 2026-09-02 ni pobrisalo.
// Posledica ni bila vidna, dokler je zagon tekel do konca: koncni Cleanup je vrstico pustil,
// naslednji zacetni Cleanup pa je na njej padel s krsitvijo tujega kljuca (SQL 547) — test se
// je s tem trajno zaklenil in ga ni odklenil noben ponoven zagon, samo rocni poseg v bazo.
async Task Cleanup()=>await Sql("DELETE ops.AlertDelivery WHERE AlertId IN(SELECT AlertId FROM ops.Alert WHERE OrganizationId=@Org); DELETE ops.Alert WHERE OrganizationId=@Org; DELETE ops.IntegrationHealth WHERE OrganizationId=@Org; DELETE ops.ErrorLog WHERE RunId IN(SELECT RunId FROM ops.PipelineRun WHERE OrganizationId=@Org); DELETE ops.PipelineRun WHERE OrganizationId=@Org; DELETE ops.ScheduleProfile WHERE OrganizationId=@Org; DELETE out.OutboxAttempt WHERE OutboxMessageId IN(SELECT OutboxMessageId FROM out.OutboxMessage WHERE OrganizationId=@Org); DELETE out.OutboxMessage WHERE OrganizationId=@Org; DELETE dbo.IntegrationProfile WHERE OrganizationId=@Org; DELETE dbo.OrganizationConfig WHERE OrganizationId=@Org;",("@Org",organizationId));
