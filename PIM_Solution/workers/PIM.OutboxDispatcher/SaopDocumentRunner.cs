    ...
    while (documents < options.MaxDocuments && !cancellationToken.IsCancellationRequested)
    {
      ...
      if (built.MissingMandatory.Count > 0)
      {
        await CompleteAsync(connection, built.MessageIds, succeeded: false, statusCode: null, body: null,
          ...
          cancellationToken);
        failed++;
        notes.Add($"{built.EntityKey}: ni poslano, manjka {string.Join(", ", built.MissingMandatory)}.");
        continue;
      }
      ...
      if (outcome.Response.IsSuccess)
      {
        ...
        sent++;
        notes.Add($"{built.EntityKey}: {shape.Operation(built.Intent)} uspešno"
      ...
      // šele ob naslednjem zagonu in ne z vrtenjem v tej zanki. Popravek se zgodi natanko
      // enkrat: če tudi druga metoda pade, gre napaka uporabniku.
      if (SaopErrorTranslator.Retry(advice) is { } correctedIntent && correctedIntent != built.Intent)
      {
        ...
        notes.Add($"{built.EntityKey}: {advice.Summary} PIM je takoj poskusil z metodo {shape.Operation(correctedIntent)}.");
        ...
        if (outcome.Response.IsSuccess)
        {
          ...
          sent++;
          notes.Add($"{built.EntityKey}: {shape.Operation(corrected.Intent)} uspešno po samopopravku"
      ...
      failed++;
      notes.Add($"{built.EntityKey}: {advice.Summary}"
