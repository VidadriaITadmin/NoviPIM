/*
  231 — ce ima potomec v premikanem poddrevesu starsa, ki ni del @Affected, mu 230 pomotoma
  postavi ParentCategoryCode na NULL (osirotelo vozlisce) namesto da ohrani obstojecega.

  Odkrito pri lastnem testiranju 230 na DAVID\MSSQL19 (ne s strani uporabnika): premik kategorije
  "tracni_sistemi" (zrcaljena veja pod videlektro/razsvetljava, glej 230) je zaradi PRESEGA vzroka
  (spodaj) s seboj potegnil se 5 kategorij "tracni_sistemi___1_fazni_*", ki so DEJANSKO otroci
  DRUGE, pravilno kodirane kategorije "razsvetljava___tracni_sistemi" - ne premikane "tracni_sistemi".
  Ti dve kategoriji imata namrec IDENTICNO prikazano pot "Razsvetljava > Tračni sistemi" (isto ime,
  razlicna koda - native videlektro veja in zrcaljena svetila_si veja se po nakljucju imenujeta
  enako), obseg premika (@Affected) pa se v 228/229/230 nespremenjeno racuna po BESEDILU poti
  (node.CategoryPath LIKE root.CategoryPath + ' > %'), zato je zajel obe.

  Ko so bili ti 5 "tujih" otrok enkrat v @Affected, jih je UPDATE canon.Category poskusil preslikati
  na novo ParentCategoryCode prek parentMap (LEFT JOIN @Affected AS parentMap ON ... = node.ParentCategoryCode) -
  a njihov resnicni stars ("razsvetljava___tracni_sistemi") NI del @Affected (ni bil izbran, ni
  potomec izbranega), zato je parentMap.NewCode NULL, prejsnja koda (230) pa je to NULL vrednost
  brez zascite zapisala naravnost v ParentCategoryCode - 5 kategorij je za trenutek obviselo brez
  starsa. Podatke sem takoj popravil (ParentCategoryCode/LevelNo/CategoryPath nazaj na izvirne
  vrednosti; CategoryPath prek rekurzivnega ponovnega sestavljanja iz canon.CategoryName, ker je
  sqlcmd ob prvem popravku brez -f 65001 pokvaril sumnike v besedilu - locena, ze odpravljena tezava
  s kodiranjem moje popravne skripte, ne s podatki samimi).

  Popravek tukaj: ce parentMap ne najde ujemanja (stars potomca ni del premika), ParentCategoryCode
  OSTANE nespremenjen namesto NULL - ISNULL(parentMap.NewCode, node.ParentCategoryCode). To je varovalo
  proti tej specificni napacni situaciji, NE popravek pravega vzroka (spodaj).

  POMEMBNO - locen, sirsi problem, ki NI popravljen s to migracijo: @Roots/@Affected v 228/229/230
  dolocajo "kateri otroci spadajo zraven" izkljucno po besedilu CategoryPath, ne po dejanski verigi
  ParentCategoryCode. Kadarkoli dve kategoriji v istem drevesu delita identicno prikazano pot (isto
  ime, isti neposredni oznaceni stars po imenu, a razlicna koda - kot tukaj), bo premik ALI BRISANJE
  ene od njiju napacno zajelo tudi poddrevo druge. To je obstojeca lastnost 228 (ne nekaj, kar je
  vpeljala 229/230) in vpliva tudi na canon.DeleteCategories ter canon.GetCategoryChangeImpact
  (isti vzorec). Pravi popravek bi obseg moral graditi z rekurzivnim sledenjem ParentCategoryCode,
  ne s poizvedbo po besedilu poti - vecji, tvegan poseg v vse tri procedure, namenoma izven obsega
  te migracije. Uporabnika je treba o tem obvestiti in vprasati, ali in kdaj naj se to popravi.
*/
...
BEGIN
  ...
  */
  ...
  /*
    231: ce parentMap ne najde ujemanja (potomcev DEJANSKI stars ni del @Affected - glej opombo na
    vrhu datoteke), ParentCategoryCode OSTANE nespremenjen namesto NULL.
  */
  UPDATE node
    SET ParentCategoryCode = CASE WHEN root.CategoryCode IS NOT NULL THEN @TargetParentCategoryCode ELSE ISNULL(parentMap.NewCode, node.ParentCategoryCode) END,
  ...
  SELECT m.SourceCode, m.CategoryTreeCode, m.SourcePathKey, m.CategoryCode, affected.NewCode, m.IsActive, m.IsActive, @Actor, N''premik kategorije (229/230/231)''
