/*
  197 - nehamo trajno shranjevati surov XML/payload za uspesno obdelane vrstice.

  raw.Inbox.PayloadXml je 2026-09-14 tehtal 5,68 GB na samo 1215 vrsticah s Status='Processed'
  (povprecno ~4,8 MB na vrstico - cele strani SAOP odgovora, ne le zajeti zapisi). Payload je
  potreben SAMO dokler ga map.ProcessRawInbox se ni prebral (Pending) ali dokler je vrstica
  Quarantined in ga nekdo se preiskuje (GetInboundIssueDetailAsync predogled, migracija 141
  GetSaopEndpointSnapshot). Ko vrstica uspesno preide v Processed, noben del sistema payload ne
  bere vec - zato se od zdaj ob uspehu pobrise (NULL), namesto da ostane v bazi za vedno.

  Enako za stock.LandingRecord.RawPayload: ob uspesnem stock.ApplyLandingRecord (Status='Applied')
  se RawPayload pobrise. Quarantined vrstice (obeh tabel) OHRANIJO payload - to je edini primer,
  ko ga orodja za debug se berejo.

  Stolpca sta bila NOT NULL (migraciji 007 in 018), zato ju je treba najprej narediti NULL-able.
  Spodaj se obe prizadeti proceduri (map.ProcessRawInbox, stock.ApplyLandingRecord) v celoti
  redefinirata po enakem vzorcu kot 196 - spremenjena je samo vrstica, ki oznaci uspeh.

  Enkratno ciscenje: obstojece Processed/Applied vrstice se ob tej migraciji tudi pobrisejo
  (ne samo bodoce), da se prostor dejansko sprosti.
*/

SET XACT_ABORT ON;

IF (SELECT is_nullable FROM sys.columns WHERE object_id = OBJECT_ID(N'raw.Inbox') AND name = N'PayloadXml') = 0
  ALTER TABLE raw.Inbox ALTER COLUMN PayloadXml nvarchar(max) NULL;

IF (SELECT is_nullable FROM sys.columns WHERE object_id = OBJECT_ID(N'stock.LandingRecord') AND name = N'RawPayload') = 0
  ALTER TABLE stock.LandingRecord ALTER COLUMN RawPayload nvarchar(max) NULL;
    ...
    BEGIN TRY
      ...
      */
      /*
        ...
        inspiracji® = ambientalna; ¯Gˆ¢wne zdj©cie produktowe® = glavna; ostalo = galerija),
...
BEGIN
  ...
  UPDATE stock.LandingRecord SET Status=N''Applied'',RawPayload=NULL WHERE LandingRecordId=@LandingRecordId;
END

');

UPDATE raw.Inbox SET PayloadXml = NULL WHERE Status = N'Processed' AND PayloadXml IS NOT NULL;
UPDATE stock.LandingRecord SET RawPayload = NULL WHERE Status = N'Applied' AND RawPayload IS NOT NULL;

IF (SELECT is_nullable FROM sys.columns WHERE object_id = OBJECT_ID(N'raw.Inbox') AND name = N'PayloadXml') = 0
  THROW 51496, N'197: raw.Inbox.PayloadXml ni NULL-able.', 1;
IF (SELECT is_nullable FROM sys.columns WHERE object_id = OBJECT_ID(N'stock.LandingRecord') AND name = N'RawPayload') = 0
  THROW 51497, N'197: stock.LandingRecord.RawPayload ni NULL-able.', 1;
IF OBJECT_DEFINITION(OBJECT_ID(N'map.ProcessRawInbox')) NOT LIKE N'%PayloadXml=NULL%'
  THROW 51498, N'197: map.ProcessRawInbox ne pobrise PayloadXml ob uspehu.', 1;
IF OBJECT_DEFINITION(OBJECT_ID(N'stock.ApplyLandingRecord')) NOT LIKE N'%RawPayload=NULL%'
  THROW 51499, N'197: stock.ApplyLandingRecord ne pobrise RawPayload ob uspehu.', 1;
IF EXISTS (SELECT 1 FROM raw.Inbox WHERE Status = N'Processed' AND PayloadXml IS NOT NULL)
  THROW 51500, N'197: se vedno obstajajo Processed vrstice s shranjenim PayloadXml.', 1;
