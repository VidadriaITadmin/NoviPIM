/*
  057 — trgovinski podatki iz SAOP: canon.ProductCommercial se koncno polni.

  Ugotovljeno z merjenjem 21. 8. 2026. GetItemsGeneralData vrne 69 elementov na artikel,
  mi jih beremo 12. Cel sklop PropertiesData smo zavrgli — in prav tam so teze, volumen,
  mere pakiranja, kolicine pakiranja in drzava porekla. Carinska tarifa je v
  GeneralData/CustomsTariffNo in je izpolnjena pri 93 % artiklov.

  Posledica je bila merljiva: canon.ProductCommercial je imela 0 vrstic (edina je bila iz
  testa), validacijska profila ERP_L1_EU in ERP_L1_THIRD sta bila pri 0 % veljavnih in
  COMMERCIAL_L2 prav tako. Ni slo za manjkajoc vir; slo je za manjkajoco preslikavo.

  Kaj naredi ta migracija:
    1. doda stolpce, ki jih tabela se ni imela: Volume, PackageLength, PackageWidth,
       PackageHeight, DimensionUnit;
    2. doda 11 preslikav na vseh SAOP konektorjih;
    3. razsiri map.ProcessRawInbox z MERGE v canon.ProductCommercial — brez tega bi
       preslikave polnile map.ExtractedValue in obticale tam;
    4. razsiri canon.FieldValue, da validacija nova polja sploh vidi;
    5. vklopi stiri validacijske zahteve, ki so od migracije 047 cakale z IsActive = 0
       (volumen in mere pakiranja) — zdaj imajo polje.

  Cesar ta migracija NE naredi: ne dotakne se ze zajetih strani. Podatek je v raw.Inbox,
  a te strani so ze Processed. Da trgovinski podatki pridejo v katalog, je treba pognati
  zajem znova (--full) ali preslikati obstojece zagone.

  Migrator ne pozna locila GO, zato so pogled, procedura in stavki nad pravkar dodanimi
  stolpci zaviti v EXEC(N'...').
*/

SET XACT_ABORT ON;

/* --- 1) manjkajoci stolpci --------------------------------------------- */

IF COL_LENGTH(N'canon.ProductCommercial', N'Volume') IS NULL
  ALTER TABLE canon.ProductCommercial ADD
    Volume decimal(19,4) NULL,
    PackageLength decimal(19,4) NULL,
    PackageWidth decimal(19,4) NULL,
    PackageHeight decimal(19,4) NULL,
    DimensionUnit nvarchar(40) NULL;

/* --- 2) preslikave na vseh SAOP konektorjih ----------------------------- */

MERGE map.FieldMapping AS target
USING
(
  SELECT connector.SourceConnectorId, N'ItemGeneralData' AS EntityType,
         source.SourceElement, source.TargetFieldCode
  FROM map.SourceConnector connector
  CROSS JOIN (VALUES
    (N'PropertiesData/ItemGrossWeight/text()[1]', N'ProductCommercial.GrossWeight'),
    (N'PropertiesData/ItemWeightPerUnit/text()[1]', N'ProductCommercial.NetWeight'),
    (N'PropertiesData/ItemVolumePerUnit/text()[1]', N'ProductCommercial.Volume'),
    (N'PropertiesData/ItemLength/text()[1]', N'ProductCommercial.PackageLength'),
    (N'PropertiesData/ItemWidth/text()[1]', N'ProductCommercial.PackageWidth'),
    (N'PropertiesData/ItemHeight/text()[1]', N'ProductCommercial.PackageHeight'),
    (N'PropertiesData/ItemDimensionUOM/text()[1]', N'ProductCommercial.DimensionUnit'),
    (N'PropertiesData/ItemQuantityOfPackaging/text()[1]', N'ProductCommercial.Pak1'),
    (N'PropertiesData/ItemQuantityOfPackaging2/text()[1]', N'ProductCommercial.Pak2'),
    (N'PropertiesData/ItemCountryOfOrigin/text()[1]', N'ProductCommercial.CountryOfOrigin'),
    (N'GeneralData/CustomsTariffNo/text()[1]', N'ProductCommercial.CustomsTariff')
...
/* --- 3) canon.FieldValue vidi nova polja -------------------------------- */
...
/* --- 4) map.ProcessRawInbox pise tudi trgovinske podatke ---------------- */
...
/* --- 5) vklopi zahteve, ki so cakale na polje --------------------------- */

EXEC(N'
UPDATE requirement
SET IsActive = 1
FROM val.FieldRequirement requirement
WHERE requirement.IsActive = 0
  AND requirement.FieldCode IN (N''ProductCommercial.Volume'', N''ProductCommercial.PackageLength'',
                                N''ProductCommercial.PackageWidth'', N''ProductCommercial.PackageHeight'');
');

/* --- 6) varovalke ------------------------------------------------------- */

IF (SELECT COUNT(*) FROM map.FieldMapping m
    JOIN map.SourceConnector c ON c.SourceConnectorId = m.SourceConnectorId
    WHERE c.ConnectorType = N'SAOP' AND m.IsActive = 1
      AND m.TargetFieldCode LIKE N'ProductCommercial.%') < 11
  THROW 52370, 'Preslikave za trgovinske podatke niso nastale.', 1;

IF COL_LENGTH(N'canon.ProductCommercial', N'DimensionUnit') IS NULL
  THROW 52371, 'Stolpci za mere pakiranja niso nastali.', 1;
