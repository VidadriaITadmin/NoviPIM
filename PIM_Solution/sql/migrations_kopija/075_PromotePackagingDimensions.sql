/*
  075 — objava nese tudi volumen in mere pakiranja.

  Migracija 074 je stolpcem predloge dala vir, izvoz pa je padel z 'Invalid column name': mere
  so v canon.ProductCommercial (od 057), v pim.ProductCommercial pa jih ni bilo. Objava je nesla
  samo tisto, kar je tabela poznala prej, in izvoz bere objavo.

  Isti razred napake kot pri 058 (objava ni bila koncana), le eno nadstropje nizje: tam so
  manjkale cele otroske tabele, tu pet stolpcev v eni.
*/

SET XACT_ABORT ON;

IF COL_LENGTH(N'pim.ProductCommercial', N'Volume') IS NULL
  ALTER TABLE pim.ProductCommercial ADD Volume decimal(19,6) NULL;
IF COL_LENGTH(N'pim.ProductCommercial', N'PackageLength') IS NULL
  ALTER TABLE pim.ProductCommercial ADD PackageLength decimal(19,4) NULL;
IF COL_LENGTH(N'pim.ProductCommercial', N'PackageWidth') IS NULL
  ALTER TABLE pim.ProductCommercial ADD PackageWidth decimal(19,4) NULL;
IF COL_LENGTH(N'pim.ProductCommercial', N'PackageHeight') IS NULL
  ALTER TABLE pim.ProductCommercial ADD PackageHeight decimal(19,4) NULL;
IF COL_LENGTH(N'pim.ProductCommercial', N'DimensionUnit') IS NULL
  ALTER TABLE pim.ProductCommercial ADD DimensionUnit nvarchar(20) NULL;
...
IF COL_LENGTH(N'pim.ProductCommercial', N'DimensionUnit') IS NULL
  THROW 52751, 'Objava se vedno ne pozna enote mer.', 1;

IF NOT EXISTS
(
  SELECT 1 FROM sys.sql_modules
  WHERE object_id = OBJECT_ID(N'val.Promote') AND definition LIKE N'%PackageHeight%'
)
  THROW 52752, 'val.Promote ne objavlja mer pakiranja.', 1;
