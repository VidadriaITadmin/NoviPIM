...
MERGE out.ExportProfile target USING(VALUES(N'CUSTOMERS_B2B',N'B2B stranke',N'B2B',N'CUSTOMERS',1),(N'PRODUCTS_B2B',N'B2B izdelki',N'B2B',N'PRODUCTS',1),(N'SHIPPING_B2B',N'B2B pravila dostave',N'B2B',N'SHIPPING',1)) source(ProfileCode,Name,ChannelCode,EntityType,IsActive)
ON target.ProfileCode COLLATE Latin1_General_BIN2=source.ProfileCode COLLATE Latin1_General_BIN2 WHEN MATCHED THEN UPDATE SET Name=source.Name,ChannelCode=source.ChannelCode,EntityType=source.EntityType,IsActive=source.IsActive,UpdatedUtc=SYSUTCDATETIME() WHEN NOT MATCHED THEN INSERT(ProfileCode,Name,ChannelCode,EntityType,IsActive) VALUES(source.ProfileCode,source.Name,source.ChannelCode,source.EntityType,source.IsActive);
MERGE out.ExportColumn target USING(SELECT p.ExportProfileId,v.ColumnCode,v.OutputColumnName,v.CanonicalFieldCode,v.SortOrder,v.IsRequired,CONVERT(bit,1) IsActive FROM (VALUES
 (N'CUSTOMERS_B2B',N'KEY',N'customer_key',N'Customer.Key',10,1),(N'CUSTOMERS_B2B',N'GROUP',N'customer_group',N'Customer.MagentoGroupKey',20,1),(N'CUSTOMERS_B2B',N'TYPE',N'customer_type',N'Customer.Type',30,0),(N'CUSTOMERS_B2B',N'FLAGS',N'policy_flags',N'Customer.Flags',40,0),(N'CUSTOMERS_B2B',N'PRICE_LIST',N'price_list',N'Customer.PriceList',50,0),(N'CUSTOMERS_B2B',N'DISCOUNT_PRICE_LIST',N'discount_price_list',N'Customer.DiscountPriceList',60,0),(N'CUSTOMERS_B2B',N'PAYER',N'payer',N'Customer.Payer',70,0),(N'CUSTOMERS_B2B',N'TIERS',N'value_tiers',N'Customer.ValueTiers',80,0),(N'CUSTOMERS_B2B',N'GROUP_DISCOUNTS',N'group_discounts',N'Customer.GroupDiscounts',90,0),(N'CUSTOMERS_B2B',N'B2B_PLUS_FROM',N'b2b_plus_valid_from',N'Customer.B2bPlusValidFrom',100,0),(N'CUSTOMERS_B2B',N'B2B_PLUS_TO',N'b2b_plus_valid_to',N'Customer.B2bPlusValidTo',110,0),(N'CUSTOMERS_B2B',N'SHIPPING_POLICY',N'shipping_policy',N'Policy.Shipping',120,1),(N'CUSTOMERS_B2B',N'WEB_POLICY',N'web_discount_percent',N'Policy.B2bWebPercent',130,1),
 ...
 ,(N'SHIPPING_B2B',N'RULE',N'rule_code',N'Shipping.RuleCode',10,1),(N'SHIPPING_B2B',N'ORDER_THRESHOLD',N'order_threshold',N'Shipping.OrderThreshold',20,0),(N'SHIPPING_B2B',N'PACKAGE_LENGTH',N'package_length_m',N'Shipping.PackageLengthMeters',30,0),(N'SHIPPING_B2B',N'NET',N'shipping_net',N'Shipping.Net',40,1),(N'SHIPPING_B2B',N'FREE',N'is_free',N'Shipping.IsFree',50,1),(N'SHIPPING_B2B',N'PRIORITY',N'priority',N'Shipping.Priority',60,1)
) v(ProfileCode,ColumnCode,OutputColumnName,CanonicalFieldCode,SortOrder,IsRequired) JOIN out.ExportProfile p ON p.ProfileCode COLLATE Latin1_General_BIN2=v.ProfileCode COLLATE Latin1_General_BIN2) source
ON target.ExportProfileId=source.ExportProfileId AND target.ColumnCode COLLATE Latin1_General_BIN2=source.ColumnCode COLLATE Latin1_General_BIN2 WHEN MATCHED THEN UPDATE SET OutputColumnName=source.OutputColumnName,CanonicalFieldCode=source.CanonicalFieldCode,SortOrder=source.SortOrder,IsRequired=source.IsRequired,IsActive=source.IsActive WHEN NOT MATCHED THEN INSERT(ExportProfileId,ColumnCode,OutputColumnName,CanonicalFieldCode,SortOrder,IsRequired,IsActive) VALUES(source.ExportProfileId,source.ColumnCode,source.OutputColumnName,source.CanonicalFieldCode,source.SortOrder,source.IsRequired,source.IsActive);

MERGE sec.Role target USING(VALUES(N'COMMERCIAL',N'Urednik komerciale')) source(RoleCode,Name) ON target.RoleCode COLLATE Latin1_General_BIN2=source.RoleCode COLLATE Latin1_General_BIN2 WHEN MATCHED THEN UPDATE SET Name=source.Name WHEN NOT MATCHED THEN INSERT(RoleCode,Name) VALUES(source.RoleCode,source.Name);
MERGE sec.NavigationGroup target USING(VALUES(N'PRAVILA',N'Pravila',30,1)) source(GroupCode,Name,SortOrder,IsActive) ON target.GroupCode COLLATE Latin1_General_BIN2=source.GroupCode COLLATE Latin1_General_BIN2 WHEN MATCHED THEN UPDATE SET Name=source.Name,SortOrder=source.SortOrder,IsActive=source.IsActive WHEN NOT MATCHED THEN INSERT(GroupCode,Name,SortOrder,IsActive) VALUES(source.GroupCode,source.Name,source.SortOrder,source.IsActive);
MERGE sec.NavigationItem target USING(SELECT g.NavigationGroupId,v.ItemCode,v.Name,v.Route,v.SortOrder FROM (VALUES(N'CUSTOMERS_B2B',N'Stranke',N'/stranke',10),(N'DISCOUNT_RULES_B2B',N'Pravila popustov',N'/pravila-popustov',20)) v(ItemCode,Name,Route,SortOrder) CROSS JOIN sec.NavigationGroup g WHERE g.GroupCode COLLATE Latin1_General_BIN2=N'PRAVILA' COLLATE Latin1_General_BIN2) source ON target.ItemCode COLLATE Latin1_General_BIN2=source.ItemCode COLLATE Latin1_General_BIN2 WHEN MATCHED THEN UPDATE SET NavigationGroupId=source.NavigationGroupId,Name=source.Name,Route=source.Route,SortOrder=source.SortOrder,IsActive=1 WHEN NOT MATCHED THEN INSERT(NavigationGroupId,ItemCode,Name,Route,SortOrder,IsActive) VALUES(source.NavigationGroupId,source.ItemCode,source.Name,source.Route,source.SortOrder,1);
...
BEGIN SET NOCOUNT ON; SET XACT_ABORT ON;
 ...
 CREATE TABLE #Mapped(TargetField nvarchar(200) COLLATE Latin1_General_BIN2 NOT NULL PRIMARY KEY,Value nvarchar(max) NULL);
 INSERT #Mapped(TargetField,Value)
 SELECT m.TargetField,j.value FROM OPENJSON(@payload) j JOIN map.B2bFieldMapping m ON m.SourceCode COLLATE Latin1_General_BIN2=@source COLLATE Latin1_General_BIN2 AND m.EntityType COLLATE Latin1_General_BIN2=@entity COLLATE Latin1_General_BIN2 AND m.SourceField COLLATE Latin1_General_BIN2=j.[key] COLLATE Latin1_General_BIN2 AND m.IsActive=1;
 ...
 WHERE NOT EXISTS(SELECT 1 FROM map.B2bFieldMapping m WHERE m.SourceCode COLLATE Latin1_General_BIN2=@source COLLATE Latin1_General_BIN2 AND m.EntityType COLLATE Latin1_General_BIN2=@entity COLLATE Latin1_General_BIN2 AND m.SourceField COLLATE Latin1_General_BIN2=j.[key] COLLATE Latin1_General_BIN2 AND m.IsActive=1);
 INSERT b2b.MappingRejection(LandingRecordId,SourceField,ReasonCode,ReasonDetail)
 ...
 LEFT JOIN OPENJSON(@payload) j ON j.[key] COLLATE Latin1_General_BIN2=m.SourceField COLLATE Latin1_General_BIN2
 WHERE m.SourceCode COLLATE Latin1_General_BIN2=@source COLLATE Latin1_General_BIN2 AND m.EntityType COLLATE Latin1_General_BIN2=@entity COLLATE Latin1_General_BIN2 AND m.IsActive=1 AND m.IsRequired=1 AND NULLIF(LTRIM(RTRIM(j.value)),N'''') IS NULL;
...
EXEC(N'CREATE OR ALTER PROCEDURE b2b.SaveCustomerTypeMapping @OrganizationId int,@CustomerTypeCode nvarchar(60),@MagentoGroupKey nvarchar(100)=NULL,@ChangedBy nvarchar(200) AS BEGIN SET NOCOUNT ON;SET XACT_ABORT ON;BEGIN TRAN;IF NOT EXISTS(SELECT 1 FROM pim.CustomerTypeCatalog WHERE CustomerTypeCode=@CustomerTypeCode) THROW 52001,N''Tip stranke ne obstaja.'',1;DECLARE @old nvarchar(max)=(SELECT * FROM pim.CustomerTypeMagentoGroup WHERE CustomerTypeCode=@CustomerTypeCode FOR JSON PATH,WITHOUT_ARRAY_WRAPPER);UPDATE pim.CustomerTypeMagentoGroup SET MagentoGroupKey=NULLIF(@MagentoGroupKey,N''''),UpdatedUtc=SYSUTCDATETIME() WHERE CustomerTypeCode=@CustomerTypeCode;INSERT b2b.AuditLog(OrganizationId,EntityType,EntityKey,ActionCode,OldValueJson,NewValueJson,ChangedBy) SELECT @OrganizationId,N''CustomerTypeMagentoGroup'',@CustomerTypeCode,N''UPDATE'',@old,(SELECT * FROM pim.CustomerTypeMagentoGroup WHERE CustomerTypeCode=@CustomerTypeCode FOR JSON PATH,WITHOUT_ARRAY_WRAPPER),@ChangedBy;COMMIT;END');
EXEC(N'CREATE OR ALTER PROCEDURE b2b.SaveCustomerValueTier @OrganizationId int,@CustomerId bigint,@TierNumber tinyint,@ThresholdGrossExVat decimal(19,4),@PercentValue decimal(9,4),@ChangedBy nvarchar(200) AS BEGIN SET NOCOUNT ON;SET XACT_ABORT ON;BEGIN TRAN;IF NOT EXISTS(SELECT 1 FROM b2b.Customer WHERE CustomerId=@CustomerId AND OrganizationId=@OrganizationId) THROW 52000,N''Stranka ne obstaja.'',1;DECLARE @old nvarchar(max)=(SELECT * FROM pim.CustomerValueDiscountTier WHERE CustomerId=@CustomerId AND TierNumber=@TierNumber FOR JSON PATH,WITHOUT_ARRAY_WRAPPER);MERGE pim.CustomerValueDiscountTier t USING(SELECT @CustomerId CustomerId,@TierNumber TierNumber) s ON t.CustomerId=s.CustomerId AND t.TierNumber=s.TierNumber WHEN MATCHED THEN UPDATE SET ThresholdGrossExVat=@ThresholdGrossExVat,PercentValue=@PercentValue,IsActive=1 WHEN NOT MATCHED THEN INSERT(CustomerId,TierNumber,ThresholdGrossExVat,PercentValue) VALUES(@CustomerId,@TierNumber,@ThresholdGrossExVat,@PercentValue);INSERT b2b.AuditLog(OrganizationId,EntityType,EntityKey,ActionCode,OldValueJson,NewValueJson,ChangedBy) SELECT @OrganizationId,N''CustomerValueTier'',CONCAT(@CustomerId,N'':'',@TierNumber),N''UPSERT'',@old,(SELECT * FROM pim.CustomerValueDiscountTier WHERE CustomerId=@CustomerId AND TierNumber=@TierNumber FOR JSON PATH,WITHOUT_ARRAY_WRAPPER),@ChangedBy;COMMIT;END');
EXEC(N'CREATE OR ALTER PROCEDURE b2b.SaveValueDiscountTier @OrganizationId int,@TierNumber tinyint,@ThresholdGrossExVat decimal(19,4),@PercentValue decimal(9,4),@ChangedBy nvarchar(200) AS BEGIN SET NOCOUNT ON;SET XACT_ABORT ON;BEGIN TRAN;DECLARE @old nvarchar(max)=(SELECT * FROM pim.ValueDiscountTier WHERE TierNumber=@TierNumber FOR JSON PATH,WITHOUT_ARRAY_WRAPPER);UPDATE pim.ValueDiscountTier SET ThresholdGrossExVat=@ThresholdGrossExVat,PercentValue=@PercentValue,UpdatedUtc=SYSUTCDATETIME() WHERE TierNumber=@TierNumber;IF @@ROWCOUNT=0 THROW 52002,N''Vrednostni prag ne obstaja.'',1;INSERT b2b.AuditLog(OrganizationId,EntityType,EntityKey,ActionCode,OldValueJson,NewValueJson,ChangedBy) SELECT @OrganizationId,N''ValueDiscountTier'',CONVERT(nvarchar(10),@TierNumber),N''UPDATE'',@old,(SELECT * FROM pim.ValueDiscountTier WHERE TierNumber=@TierNumber FOR JSON PATH,WITHOUT_ARRAY_WRAPPER),@ChangedBy;COMMIT;END');
EXEC(N'CREATE OR ALTER PROCEDURE b2b.SaveGroupDiscountOverride @OrganizationId int,@TargetKind nvarchar(20),@CustomerId bigint=NULL,@CustomerTypeCode nvarchar(60)=NULL,@ItemGroupCode nvarchar(100),@PercentValue decimal(9,4),@ValidFrom date=NULL,@ValidTo date=NULL,@ChangedBy nvarchar(200) AS BEGIN SET NOCOUNT ON;SET XACT_ABORT ON;BEGIN TRAN;INSERT b2b.GroupDiscountOverride(OrganizationId,TargetKind,CustomerId,CustomerTypeCode,ItemGroupCode,PercentValue,ValidFrom,ValidTo) VALUES(@OrganizationId,@TargetKind,@CustomerId,@CustomerTypeCode,@ItemGroupCode,@PercentValue,@ValidFrom,@ValidTo);DECLARE @id bigint=SCOPE_IDENTITY();INSERT b2b.AuditLog(OrganizationId,EntityType,EntityKey,ActionCode,NewValueJson,ChangedBy) SELECT @OrganizationId,N''GroupDiscountOverride'',CONVERT(nvarchar(30),@id),N''INSERT'',(SELECT * FROM b2b.GroupDiscountOverride WHERE OverrideId=@id FOR JSON PATH,WITHOUT_ARRAY_WRAPPER),@ChangedBy;COMMIT;END');
...
EXEC(N'CREATE OR ALTER PROCEDURE intranet.GetCustomerTypes AS BEGIN SET NOCOUNT ON; SELECT t.CustomerTypeCode,t.Name,g.MagentoGroupKey FROM pim.CustomerTypeCatalog t LEFT JOIN pim.CustomerTypeMagentoGroup g ON g.CustomerTypeCode=t.CustomerTypeCode WHERE t.IsActive=1 ORDER BY t.Name; END');
EXEC(N'CREATE OR ALTER PROCEDURE intranet.GetValueDiscountTiers AS BEGIN SET NOCOUNT ON; SELECT TierNumber,ThresholdGrossExVat,PercentValue FROM pim.ValueDiscountTier WHERE IsActive=1 ORDER BY TierNumber; END');
EXEC(N'CREATE OR ALTER PROCEDURE intranet.GetGroupDiscountOverrides @OrganizationId int AS BEGIN SET NOCOUNT ON; SELECT o.OverrideId,o.TargetKind,c.CustomerKey,o.CustomerTypeCode,o.ItemGroupCode,o.PercentValue,o.ValidFrom,o.ValidTo FROM b2b.GroupDiscountOverride o LEFT JOIN b2b.Customer c ON c.CustomerId=o.CustomerId WHERE o.OrganizationId=@OrganizationId AND o.IsActive=1 ORDER BY o.ItemGroupCode,o.OverrideId; END');
...
EXEC(N'CREATE OR ALTER PROCEDURE out.ExportB2bShippingCsv AS BEGIN SET NOCOUNT ON; SELECT RuleCode,OrderThreshold,PackageLengthMeters,ShippingNet,IsFree,Priority FROM pim.ShippingRuleCatalog WHERE IsActive=1 ORDER BY Priority,RuleCode; END');
