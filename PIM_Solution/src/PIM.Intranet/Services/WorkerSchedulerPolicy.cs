...
/// </summary>
public sealed record CycleGroup(string Name, IReadOnlyList<CycleStep> Steps);
...
public static class WorkerCycles
{
  ...
  [
    new(Zaloga, "Zaloga in cene",
      "Vsakih 5 minut: Nowodvorski FTP in Braytron XML, zaloga iz SAOP, cene iz SAOP, izvoz cen in zaloge za splet ter osvežen katalog.csv. Datumi dobave iz SAOP tečejo po svojem razporedu (30 min).",
      10, 300, null, "zaloga-", WorkerJobReach.ExternalCall, "kliče SAOP in dobavitelja",
      ["SOURCE_FETCH", "STOCK_FILE", "SAOP_STOCK", "SAOP_PRICES", "MAGENTO_STOCK_PRICES", "MAGENTO_PRODUCTS", "SAOP_DELIVERY"]),
    new(Katalog, "Katalog za splet",
      "Vsako uro: katalog iz SAOP (delta), naročila iz SAOP, validacija in objava vseh podjetij ter poln izvoz kataloga in strank.",
      20, 3600, null, "katalog-", WorkerJobReach.ExternalCall, "kliče SAOP",
      ["SAOP_PRODUCTS", "SAOP_ORDERS_VNK", "SAOP_ORDERS_VND", "MAGENTO_PRODUCTS"]),
    ...
    new(NocniTok, "Nočni tok",
      "Vsak dan ob 02:30: cel vhodni tok — SAOP katalog (poln 1. v mesecu), prevzem, dobaviteljev XML, preslikava zaostanka, zaloge, SAOP zaloga in datumi dobave, validacija, objava, izvoz in dnevni mail o zalogi pod MID.",
      40, null, new TimeOnly(2, 30), "nocno_", WorkerJobReach.ExternalCall, "kliče SAOP in dobavitelja, pošlje e-pošto",
      ["SAOP_PRODUCTS", "SOURCE_FETCH", "GENERIC_XML", "STOCK_FILE", "SAOP_STOCK", "SAOP_DELIVERY", "MAGENTO_PRODUCTS", "STOCK_REPLENISHMENT_DIGEST"]),
  ...
  static IReadOnlyList<CycleGroup> PlanZaloga(CycleOptions options, CycleEnvironment env)
  {
    ...
    // katalog.csv in stranke.csv s svežimi cenami in zalogo, brez validacije (ta je na uro v katalogu).
    // Samo podjetje kataloga: do 2026-09-15 je tekel za vsa štiri in bil vzrok zastojev na val.RunValidation.
    if (env.Organizations.Contains(env.CatalogOrganizationId))
      groups.Add(new($"Osvezitev kataloga in strank s cenami in zalogo (podjetje {env.CatalogOrganizationId})",
        [Worker("Katalog in stranke", "PIM.B2bWorker", ["--export-magento", "--organization-id", Org(env.CatalogOrganizationId)], env.Paths, env.CatalogOrganizationId)]));
  ...
  static IReadOnlyList<CycleGroup> PlanKatalog(CycleEnvironment env)
  {
    ...
    if (env.Organizations.Contains(env.CatalogOrganizationId))
      groups.Add(new($"Izvoz kataloga in strank (podjetje {env.CatalogOrganizationId})",
        [Worker("Katalog in stranke", "PIM.B2bWorker", ["--export-magento", "--organization-id", Org(env.CatalogOrganizationId)], env.Paths, env.CatalogOrganizationId)]));
    return groups;
  }
  ...
  static IReadOnlyList<CycleGroup> PlanNocniTok(CycleOptions options, CycleEnvironment env)
  {
    ...
    // 8. Izvoz iz objave: en par datotek, samo podjetje kataloga.
    if (env.Organizations.Contains(env.CatalogOrganizationId))
      groups.Add(new("Izvoz kataloga in strank",
        [Worker("Katalog in stranke", "PIM.B2bWorker", ["--export-magento", "--organization-id", Org(env.CatalogOrganizationId)], env.Paths, env.CatalogOrganizationId)]));
  ...
  static IReadOnlyList<CycleGroup> PlanSamotest(CycleEnvironment env)
  {
  ...
  static IReadOnlyList<CycleStep> StockFilesSteps(string vir, string mapa, CycleEnvironment env, string[] urnik)
  {
  ...
  static IReadOnlyList<CycleStep> XmlSteps(string sourceCode, string fixtureFolder, CycleEnvironment env)
  {
  ...
  static CycleStep ValidateAndPromote(int org) =>
  ...
  static CycleStep Worker(string name, string worker, IReadOnlyList<string> arguments, WorkerPaths paths, int? org = null,
  ...
  static CycleStep Note(string name, string note) => new(name, CycleStepKind.Note, note);

  /// <summary>Kadar je register EXPORT_ROOT nastavljen, worker mapo razreši sam; sicer ista mapa kot v skripti.</summary>
  static string[] ExportDirectory(CycleEnvironment env, int org) =>
    env.ExportRoot is { Length: > 0 } ? [] : ["--output-dir", env.Paths.ExportDirectory(org)];

  static string Orgs(CycleEnvironment env) => string.Join(",", env.Organizations.Select(Org));
  static string Org(int value) => value.ToString(CultureInfo.InvariantCulture);
}
