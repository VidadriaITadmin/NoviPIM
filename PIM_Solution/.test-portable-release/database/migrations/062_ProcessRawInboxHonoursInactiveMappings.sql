/*
  061 — izklopljena preslikava mora nehati pisati v katalog.

  Kaj se je zgodilo. Migracija 059 je izklopila preslikavo, ki je pisala dobaviteljevo
  kategorijo prve ravni ('Interior lighting') pod spletno stran 'B2C', migracija 060 pa je
  vrstice, ki jih je ta preslikava naredila, pobrisala. Ob prvi ponovni preslikavi istega
  zajema so se vrnile.

  Zakaj. map.ProcessRawInbox bere map.ExtractedValue in ne pogleda, ali je preslikava, ki je
  vrednost izlusila, se aktivna. Ze izluscene vrednosti ostanejo v tabeli (namenoma — so
  sled, kaj je vir poslal), zato je izklopljena preslikava se naprej pisala v katalog.
  IsActive je bil s tem samo napol resnicen: novih vrednosti ni vec luscil, stare pa so
  tekle naprej.

  Kaj se popravi. Vseh pet mest, kjer postopek bere vrednosti za vpis v katalog (identiteta
  zapisa, preverba cene, zmagovalne vrednosti, kategorije in cene), zahteva aktivno
  preslikavo. Nespremenjeno ostane samo zadnje branje — tisto, ki ob karanteni v
  map.UnmappedValue zapise, kaj vse je vhodna vrstica nosila; tam je pravilno, da se vidi
  vse, tudi vrednosti izklopljenih preslikav.

  Ob tem se ponovno pobrisejo vrstice, ki jih je izklopljena preslikava medtem vrnila.
*/
...
/* --- vrstice izklopljene preslikave ven, tokrat dokoncno ------------------ */
...
IF (SELECT COUNT(*) FROM sys.sql_modules
    WHERE object_id = OBJECT_ID(N'map.ProcessRawInbox')
      AND definition LIKE N'%mapping.IsActive=1)%') <> 1
  THROW 52611, 'map.ProcessRawInbox ne preverja aktivne preslikave.', 1;
...
(
  ...
  THROW 52612, 'Dobaviteljeve kategorije pod B2C so se vedno v katalogu.', 1;
