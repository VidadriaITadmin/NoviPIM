/*
  067 — odsotnost vrstice v registru pomeni izdelek, ne "preskoci".

  Migracija 064 je map.ProcessRawInbox naucila lociti sifrant od izdelka, a je pogoj zapisala
  narobe: zahtevala je vrstico map.EntityMapping s TargetDomain = 'Product'. Doslej je postopek
  obdelal vsako vhodno vrstico, ki je imela izluscene vrednosti, ne glede na register.

  Razlika se pokaze tam, kjer vrednosti nastanejo brez vrstice v registru — na primer v
  PIM.F3.Integration, ki mnozicno obdelavo dokazuje z rocno vstavljenimi vrednostmi. Po 064 je
  taka stran ostala neobdelana in test je padel s 'Vhodna vrstica ni bila obdelana'.

  Popravek je obrnjen pogoj: preskoci se samo tisto, kar je v registru IZRECNO oznaceno kot drug
  svet (TargetDomain <> 'Product'). Odsotnost vrstice pomeni izdelek, enako kot privzetek stolpca
  in enako kot pred 064. Skladisca se se naprej preskocijo, ker imajo TargetDomain = 'Warehouse'.
*/
...
IF (SELECT COUNT(*) FROM sys.sql_modules
  ...
  THROW 52671, 'map.ProcessRawInbox ne preskoci sifranta po registru.', 1;

IF (SELECT COUNT(*) FROM sys.sql_modules
    WHERE object_id = OBJECT_ID(N'map.ProcessRawInbox') AND definition LIKE N'%TargetDomain=''Product''%') <> 0
  THROW 52672, 'V map.ProcessRawInbox je ostal stari, prestrogi pogoj.', 1;
